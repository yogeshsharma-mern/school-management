...
// Updated fetch function with pagination parameters
const fetchTransactions = async (page = 1, limit = 10) => {
  const res = await apiGet(`${apiPath.walletTransactions}?page=${page}&limit=${limit}`);
  return res;
};
...
export default function TransactionDetails() {
  ...
  const {
    ...
    queryKey: ['wallet-transactions', currentPage, pageSize],
    queryFn: () => fetchTransactions(currentPage, pageSize),
  });

  // Extract transactions and pagination info from response
  ...
  // Filter and sort transactions (client-side for current page)
  const filteredTransactions = React.useMemo(() => {
    let filtered = transactions.filter(tx => {
      const matchesSearch = 
        tx.referenceId?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <div className="md:p-6 p-2 space-y-6 w-[100vw] md:w-[83vw]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <button 
            onClick={() => navigate(-1)}
            className='bg-gray-300 px-3 py-1 rounded mb-3 cursor-pointer hover:bg-gray-400 transition-colors'
          >
            Back
          </button>
          <h1 className="text-2xl font-bold text-gray-900">Transaction History</h1>
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-1">
                <button
                  ...
                  className={`p-1.5 rounded ${hasPrevious ? 'hover:bg-gray-200 text-gray-600' : 'text-gray-300 cursor-not-allowed'}`}
                >
                ...
                <button
                  onClick={() => handlePageChange(pagination.page + 1)}
                  disabled={!hasNext}
                  className={`p-1.5 rounded ${hasNext ? 'hover:bg-gray-200 text-gray-600' : 'text-gray-300 cursor-not-allowed'}`}
                >
