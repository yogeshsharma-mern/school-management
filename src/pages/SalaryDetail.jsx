...
export default function SalaryDetail() {
  ...
  const { data, isLoading, isFetching, error } = useQuery({
    queryKey: ["teacherTransactions", id, year],
    queryFn: () =>
      apiGet(`/admins/teachersalary/get-transction?teacherId=${id}&year=${year}`),
    enabled: !!id,
  });
  ...
  if (error) {
    toast.error("Failed to fetch salary transactions ❌");
  }
  ...
  return (
    <Box sx={{ p: 4, minHeight: "100vh", bgcolor: "#f5f6fa" }}>
      ...
      {/* No data */}
      {!isLoading && transactions.length === 0 && (
        <Typography align="center" color="text.secondary" variant="body1" mt={6}>
          No transactions found for {year}.
        </Typography>
      )}
            ...
            >
              ...
              >
                <CardContent>
                  ...
                  <Typography variant="body2" color="text.secondary" mt={1}>
                    <strong>Reference ID:</strong> {tx.referenceId || "N/A"}
                  </Typography>
                </CardContent>
              </Card>
            </motion.div>
